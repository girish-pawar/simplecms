...
{
$tableids = $db->get_row("SHOW FULL COLUMNS FROM downloads_category");
$fid = $tableids->Field;
//echo 'fid :'.$tableids->Field;

$idcount = $db->get_var("SELECT COUNT(*) FROM downloads_category");	
$getgalleries =$db->get_results("SELECT * FROM downloads_category");
if($getgalleries)
{
...
<?php
echo '';	
$getcolms = $db->get_results("SHOW FULL COLUMNS FROM downloads_category");
...
//	}
	...
	<td>
	<a href="../admin/index.php?option=component&kind=<?php echo $_GET['kind'];?>&method=<?php echo $_GET['method']; ?>&process=view&vid=<?php echo $vid;?>" title="View Download items" ><i class="icon-list"></i></a> 
