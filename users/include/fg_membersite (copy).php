...
class FGMembersite
{
    ...
    function RegisterUser()
    {
        ...
        //if(!$this->CollectRegistrationSubmission($fieldsarray))
        {
            return false;
        }
        //if(!$this->ValidateRegistrationSubmission())
        /*if($this->CollectRegistrationSubmission($formvars))
        {
            return false;
        }*/
        //$fieldsarray = $_POST['fieldsarray'];
        //echo 'formvars are:';
        //print_r($formvars);
        //$this->CollectRegistrationSubmission();
    ...
    function GetSelfScript($fsarray,$rid)
    {
    		//echo 'rid is:'.$_GET['id;'];
    		//echo $rid;
    	//echo 'hi is:';
							...
							//echo $key; 
								$validator = new FormValidator();
									...
									{ 
										...
										//$validator->addValidation("$value","req","Please fill in ".$field->title.".");
										//break 1;
									}
								//	break 1;
							}
        
        //$validator->addValidation("name","req","Please fill in kjdfashfkhk Name");
    ...
    function CollectRegistrationSubmission(&$formvars)
    {
       ...
       foreach($fieldsarray as $key=>$value)
						{
							//echo $key; 
							$formvars['fieldsarray['.$key.']'] = $this->Sanitize($value);
						}
    ...
    {
    	    echo 'in sav dadabase functyion';
    	     $fieldsarray = $_POST['fieldsarray'];
    	    if(!$this->DBLogin())
        {
            $this->HandleError("Database login failed!");
       ...
       /* if(!$this->IsFieldUnique($formvars,'email'))
        ...
        if(!$this->InsertIntoDB($fieldsarray,$_GET['id']))
        {
    ...
    function InsertIntoDB(&$formvars,$sid)
    {
    	echo 'formvars are:';
    //print_r($formvars);
                ...
                foreach($fieldsarray as $fieldarray)
														{
															...
															//echo $key .'is key and value is:'.$value.'</br>';
																	//$insert_query = $db->query("INSERT INTO field_data ('fd_id','rfid','rid','value','addedby') VALUES ('','$key','$rid','$value','')");
...
/*foreach($fieldsarray as $key=>$value)
                ...
                (
                ""
                "' . $this->SanitizeForSQL($key) . '",
                "' . $this->SanitizeForSQL($rid) . '",
