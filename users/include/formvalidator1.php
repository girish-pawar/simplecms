...
class FormValidator 
{
	...
	function ValidateObject($validator_obj,$formvariables,&$error_string)
	{
		$bret = true;

		$splitted = explode("=",$validator_obj->validator_string);
		$command = $splitted[0];
		$command_value = '';
		...
		if(isset($formvariables[$validator_obj->variable_name]))
		{
		 $input_value = $formvariables[$validator_obj->variable_name];
		}

		//echo 'input value is:'.$input_value;
		$bret = $this->ValidateCommand($command,$command_value,$input_value,
									$default_error_message,
									$validator_obj->variable_name,
									$formvariables);
	...
	function test_datatype($reg_exp,$input_value)
	{
